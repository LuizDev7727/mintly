...
Após criar o componente em `src/components/`, leia `src/stories/CLAUDE.md` e crie a story correspondente em `src/stories/<ComponentName>.stories.tsx`.

O componente **nunca** vai dentro de `src/stories/` — apenas o arquivo `.stories.tsx`.

Crie uma story por variante ou estado visual relevante do componente. Se o componente tem interação (clique, toggle, submit), adicione `play()` para testá-la.

```tsx
import type { Meta, StoryObj } from "@storybook/react-vite";
import { MeuComponente } from "@/components/meu-componente";  // importa de src/components/
...
**Não criar story quando** o componente for um wrapper trivial sem variantes nem estados visuais próprios.
...
- [ ] `{...props}` no final
- [ ] Story criada em `src/stories/<ComponentName>.stories.tsx`
