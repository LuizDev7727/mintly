...
`pnpx react-grab@latest pull` waits for new grabs and prints each as one line of
...
```bash
pnpx react-grab@latest pull --max-age 0
```
...
```bash
pnpx react-grab@latest pull --max-age 0 --wait 0
```
