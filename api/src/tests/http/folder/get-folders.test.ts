...
describe("GET [/api/organizations/:orgSlug/channels/:channelId/folders]", () => {
  test("should return 200 with folders array at root level", async () => {
    const response = await request(server.server)
      .get(`/api/organizations/${testOrg.slug}/channels/${channelId}/folders`)
      .set(authHeaders);
  ...
  test("should return parent info when navigating into a nested folder", async () => {
    const response = await request(server.server)
      .get(`/api/organizations/${testOrg.slug}/channels/${channelId}/folders`)
      .query({ folderName: subFolderTitle })
      .set(authHeaders);
  ...
  test("should return 404 when folder name does not exist", async () => {
    const response = await request(server.server)
      .get(`/api/organizations/${testOrg.slug}/channels/${channelId}/folders`)
      .query({ folderName: "non-existent-folder-xyz" })
      .set(authHeaders);
