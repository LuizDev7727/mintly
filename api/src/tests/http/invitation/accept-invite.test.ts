...
describe("POST [/api/invitations/:inviteId/accept]", () => {
  test("should return 204 when invite is accepted", async () => {
    const response = await request(server.server)
  ...
  test("should return 404 for non-existent invite", async () => {
    const response = await request(server.server)
      .post(`/api/invitations/${faker.string.uuid()}/accept`)
      .set(authHeaders);
  ...
  test("should return 403 when invite email does not match the user", async () => {
    ...
    const response = await request(server.server)
      .post(`/api/invitations/${inviteResponse.body.inviteId}/accept`)
      .set(authHeaders);
